...
class Security
{
    ...
    protected function decrypt($data, $passwordBased, $secret, $info)
    {
        ...
        $keySalt = Helpers::byteSubstr($data, 0, $keySize);
        if ($passwordBased) {
            $key = $this->pbkdf2($this->kdfHash, $secret, $keySalt, $this->derivationIterations, $keySize);
        } else {
            $key = $this->hkdf($this->kdfHash, $secret, $keySalt, $info, $keySize);
        ...
        $authKey = $this->hkdf($this->kdfHash, $key, null, $this->authKeyInfo, $keySize);
        $data = $this->validateData(Helpers::byteSubstr($data, $keySize, null), $authKey);
        if ($data === false) {
            return false;
        }

        $iv = Helpers::byteSubstr($data, 0, $blockSize);
        $encrypted = Helpers::byteSubstr($data, $blockSize, null);
    ...
    public function hkdf($algo, $inputKey, $salt = null, $info = null, $length = 0)
    {
        ...
        $hashLength = Helpers::byteLen($test);
        ...
        if ($length !== 0) {
            $outputKey = Helpers::byteSubstr($outputKey, 0, $length);
        }
        return $outputKey;
    }
    ...
    public function validateData($data, $key, $rawHash = false)
    {
        ...
        $hashLength = Helpers::byteLen($test);
        if (Helpers::byteLen($data) >= $hashLength) {
            $hash = Helpers::byteSubstr($data, 0, $hashLength);
            $pureData = Helpers::byteSubstr($data, $hashLength, null);
    ...
    public function generateRandomKey($length = 32)
    {
        ...
        if ($this->_useLibreSSL
            ...
            if ($key !== false && Helpers::byteLen($key) === $length) {
                return $key;
            }
        }
        ...
        if (is_resource($this->_randomFile)) {
            ...
            while ($stillNeed > 0) {
                ...
                $buffer .= $someBytes;
                $stillNeed -= Helpers::byteLen($someBytes);
                if ($stillNeed === 0) {
    ...
    public function compareString($expected, $actual)
    {
        $expected .= "\0";
        $actual .= "\0";
        $expectedLength = Helpers::byteLen($expected);
        $actualLength = Helpers::byteLen($actual);
